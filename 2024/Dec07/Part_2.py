...
# Function to make ternary number - from Stack Overflow https://stackoverflow.com/questions/34559663/convert-decimal-to-ternarybase3-in-python
def ternary (n):
    if n == 0:
        return '0'
    nums = []
...
def calculate(cal_list: list):
    ...
    if results_tree.get(''.join(cal_list), None) == None:
        if cal_list[-2] == '+':
            ...
            results_tree[''.join(cal_list)] = int(str(calculate(cal_list[:-2])) + cal_list[-1])
...
def find_result(cal_val_list: list, result: int):
    operators = {48: 43, 49: 42, 50: 124}
    #permutations = [list((bin(perm_num)[2:].zfill(len(cal_val_list) - 1)).translate(operators)) for perm_num in range(2**(len(cal_val_list) - 1))]

    permutations = [list((ternary(perm_num).zfill(len(cal_val_list) - 1)).translate(operators)) for perm_num in range(3**(len(cal_val_list) - 1))]
