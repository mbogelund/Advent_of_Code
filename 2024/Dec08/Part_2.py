...
def find_antinode(antenna1: tuple, antenna2:tuple, rows, cols):
    antinode_list = [antenna1, antenna2]

    dist = (antenna2[0] - antenna1[0], antenna2[1] - antenna1[1])

    factor1 = 1
    outer_antinode1 = (antenna1[0] - dist[0], antenna1[1] - dist[1])
    while outer_antinode1[0] >= 0 and outer_antinode1[0] < rows and outer_antinode1[1] >= 0 and outer_antinode1[1] < cols:
        antinode_list.append(outer_antinode1)
        outer_antinode1 = (antenna1[0] - factor1 * dist[0], antenna1[1] - factor1 * dist[1])
        factor1 += 1
    
    factor2 = 1
    outer_antinode2 = (antenna2[0] + dist[0], antenna2[1] + dist[1])
    while outer_antinode2[0] >= 0 and outer_antinode2[0] < rows and outer_antinode2[1] >= 0 and outer_antinode2[1] < cols:
        antinode_list.append(outer_antinode2)
        outer_antinode2 = (antenna2[0] + factor2 * dist[0], antenna2[1] + factor2 * dist[1])
        factor2 += 1
